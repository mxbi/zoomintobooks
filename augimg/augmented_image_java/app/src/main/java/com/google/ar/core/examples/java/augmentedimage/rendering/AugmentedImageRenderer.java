...
//  }
  ...
  public void draw(float[] viewMatrix, float[] projectionMatrix, AugmentedImage augmentedImage, Anchor centerAnchor, float[] colorCorrectionRgba) {
    ...
    final float block_edge_sizeX = 0.3415f;
    final float block_edge_sizeZ = 0.1902f;

    float blockScaleFactorX = augmentedImage.getExtentX()/block_edge_sizeX;
    float blockScaleFactorZ = augmentedImage.getExtentZ()/block_edge_sizeZ;
    ...
    Pose mozeModelLocalOffset = Pose.makeTranslation(
            -0.030624f*blockScaleFactorX,
            0.0f,
            -0.0011f*blockScaleFactorZ
    );

    anchorPose.compose(mozeModelLocalOffset).toMatrix(modelMatrix, 0);
    imageRenderer.updateModelMatrix(modelMatrix, blockScaleFactorX, blockScaleFactorX/10.0f, blockScaleFactorZ);
    imageRenderer.draw(viewMatrix, projectionMatrix, colorCorrectionRgba, tintColor);
